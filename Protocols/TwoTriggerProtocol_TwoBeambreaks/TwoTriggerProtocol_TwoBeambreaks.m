function TwoTriggerProtocol_TwoBeambreaks
...
R.thresholds = Dist2Deg*[50, 100, 150, 175]; % Thresholds in degrees, correspond to 50, 100, 150 in cm
...
if isempty(fieldnames(S))  % If chosen settings file was an empty struct, populate struct with default settings
    ...
    S.GUI.RewardLocation = 3; %cm
    S.GUI.StimulusTrigger1 = 50;
    S.GUI.StimulusTrigger2 = 70;
...
%**** define outputs
    ...
    io.reset = {'RotaryEncoder1', 2};           % reset encoder position and re-enable thresholds
...
RewardTime = GetValveTimes(S.GUI.WaterValveAmt, [1]);

%% Wait for belt reset
sma = NewStateMachine(); 
sma = AddState(sma, 'Name', 'WaitForReset',... % waits for start position to be reached, modified to just timeout to next state
    'Timer', 0,...
    'StateChangeConditions', {'Wire2Low', 'ResetPosition', 'BNC1High', 'ManualReward'},... 
    'OutputActions', {}); 
...
%     disp('__________')
    ...
    Thresholds = [RewardLocation, Trigger1, Trigger2] * Dist2Deg;
    ...
    sma = AddState(sma, 'Name', 'LapStart', ... % NewLap, look for beambreak
        'Timer', 0,...
        'StateChangeConditions', {'Wire1Low', 'ResetPosition'},...  
        'OutputActions', {});
    
     sma = AddState(sma, 'Name', 'ResetPosition', ... % send Timestamp and start trial
        'Timer', 0,...
        'StateChangeConditions', {'Tup', 'GoToLocation'},...  
        'OutputActions', {'RotaryEncoder1', 1, 'RotaryEncoder1', 2});
    
    sma = AddState(sma, 'Name', 'GoToLocation', ... % go to trigger 1 location
    'Timer', 0,...
    'StateChangeConditions', {'BNC1High', 'ManualReward', trigger1Site,'Trigger1', RewardSite, 'Reward', trigger2Site, 'Trigger2', 'Wire2Low', 'LapEnd'},...
    'OutputActions', {});
    ...
    sma = AddState(sma, 'Name', 'LapEnd',... % resets location of track
        'Timer', 0,...
        'StateChangeConditions', {'Tup', 'exit'},...
        'OutputActions', {}); 
