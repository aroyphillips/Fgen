function RewardDistanceTraining
%%% Modified 12/16/19 by RP to fit YiDing's Protocol: 
%%% Fully Controllable Release of Water, for training
...
R.thresholds = Dist2Deg*[20]; % Thresholds in degrees, correspond to 50, 100, 150 in cm
...
for currentTrial = 1:MaxTrials
    ...
    RewardDistance = (S.GUI.RewardDistance);
    
    %%  Remap R.Thresholds to desired values
    
    
    Thresholds = [RewardDistance] * Dist2Deg;
    
    fprintf('Reward Location: %g', Thresholds(1)/Dist2Deg)

    R.wrapPoint = 2160;
    R.thresholds = ceil(Thresholds); 
    R.startUSBStream('useTimer'); % Start streaming position data from the rotary encoder   

    

    % determine threshold numbers 
     RewardSite = locations(1);  % start of location window
    ...
    'Timer', 10,...
    'StateChangeConditions', {RewardSite, 'Reward'},...
    'OutputActions', {});
