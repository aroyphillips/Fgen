function OptionalOperantFixedRewardOptionalTime


%%% Provides animal reward located at user specified location if the animal
%%% licks in the specified time or distance, depending on user control. Assumed lick port is port 2.
...
numLocations = 2; % two possible reward sites 
...
if isempty(fieldnames(S))  % If chosen settings file was an empty struct, populate struct with default settings
    ...
    S.GUI.RewardLocationEnd = 150; %cm
    S.GUI.RewardLocationDurationT = 0.5; %s always, but not in use if IsDurationTime==0
    S.GUI.IsTimerOn = 1; % 1==s, 0==cm
...
%% Update the GUI data
    ...
    isDurationTime = (S.GUI.IsTimerOn);
    ...
    if and(isOperant, isDurationTime)
        fprintf('Operant Reward Location %gcm for %g seconds\n', RewardLocationStart, RewardLocationDuration)
    elseif and(isOperant, ~isDurationTime)
        fprintf('Reward Location %gcm to %gcm\n', RewardLocationStart, RewardLocationEnd)
    else
    ...
    if isOperant
        ...
        if isDurationTime
            ...
            sma = AddState(sma, 'Name', 'WaitForLick', ... %
                ...
                'StateChangeConditions', {'Tup', 'Bpod_Lapover', LickPortEvent, 'Reward', 'Wire2Low', 'ManualReward', locationEnd, 'Bpod_Lapover'},...  
                'OutputActions', {});
        else
            % distance window to lick
            sma = AddState(sma, 'Name', 'WaitForLick', ... %
                'Timer', 0,...
                'StateChangeConditions', {locationEnd, 'Bpod_Lapover', LickPortEvent, 'Reward', 'Wire2Low', 'ManualReward'},...  
                'OutputActions', {});
        end
