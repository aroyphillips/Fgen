...
// Incoming Bit Stream should look like this: '<b0,100,8,0500,0000!p0,2.00,010,7,0150,0000!t0,2.00,050,7,100,8,020,040,0000!^1,2.50,100,080,030!g0,2,120,100,10!>'
// Refer to the Matlab GUI for a break down of each parameter
...
void outputVolts(){
    ...
    if(isDigTime){
      ...
      else if ((!isDigActive || (dist < digStart)) && !hasDigPulsed){
        digPulseOn = false;
      }
    }
    else if (isDigDist){
      ...
      else if ((!isDigActive || (dist < digStart) || (dist >=(unsigned long)(digDuration+digStart))) && !hasDigPulsed){
        //Serial.print("dist < digStart : ");
        //Serial.print(dist < digStart);
        //Serial.print(" past dur: ");
        //Serial.println(dist >=(digDuration +digStart));
        digPulseOn = false;
      }
    }
    ...
    if(isTrainActive){
      ...
      // DISTANCE DURATION
      if (isTrainDist){
        if(((dist>trainStart) || hasStartedTrain)&& !doneSpiking){
      ...
      // TIME DURATION
          ...
          if ((dist>trainStart) && !doneSpiking){
            // start train
            if(!hasStartedTrain){
...
void parseDigitalData(char digit_str[]) {
  ...
  digStart = atof(strtokIndx);     // convert this part to a float
...
void parsePulseData(char pulse_str[]) {
  ...
  strtokIndx = strtok(NULL, ",");
  pulseStart = atof(strtokIndx);     // convert this part to a float
...
void parseTrainData(char train_str[]) {
  ...
  strtokIndx = strtok(NULL, ",");
  trainStart = atof(strtokIndx);     // convert this part to a float
  ...
  Serial.print(" Train Start:");
  Serial.print(trainStart);
...
void parseTriData(char tri_str[]) {
  ...
  strtokIndx = strtok(NULL, ",");
  triStart = atof(strtokIndx);     // convert this part to a float



  triEnd = triDuration + triStart;
  triUpSlope = triAmp / (triPeak-triStart);
  ...
  Serial.print("Triangle Start");
  Serial.println(triStart);
